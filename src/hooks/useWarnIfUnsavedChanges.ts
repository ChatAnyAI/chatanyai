...
import { useNavigate as useRouter, NavigateOptions, To } from 'react-router-dom';
...
export const useWarnIfUnsavedChanges = (options: {
  ...
  useEffect(() => {
    ...
    const originalPush = router;

    router = ((to: any, opt?: NavigateOptions) => {
      if (options.enabled) {
        const confirmLeave = window.confirm(
          'Changes you made may not be saved.'
        );

        if (confirmLeave) originalPush(to, opt);
      } else {
        originalPush(to, opt);
      }
    }) as typeof router;

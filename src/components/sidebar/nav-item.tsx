...
import { TooltipTC } from '@/components/editor-pro/potion-ui/tooltip';
...
export function NavItem({
                ...
                {user.id && (
                  <div>
                    ...
                    <div className="space-y-1 p-2 text-xs text-muted-foreground/90">
                      ...
                      <div>
                        {updatedAt &&
                          format(new Date(updatedAt * 1000), 'MMM d, yyyy, h:mm a')}
                      </div>
                    </div>
                  </div>
                )}
  ...
  const wrappedElement =
    mounted && href ? (
      <Link to={href} target={href.startsWith('http') ? '_blank' : ''}>
        {content}
      </Link>
    ) : (
      content
